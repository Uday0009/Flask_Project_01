...
class BookAdminView(ModelView):
    ...
    form_columns = ['id','sname','srno','seatno','price','theatre_id']
    def create_form(self):
        form = super(BookAdminView, self).create_form()
        form.threatre_id = SelectField('Theatre', coerce=str, choices=[(t.id, t.tname) for t in Theatre.query.all()])
        return form
class TAdminView(ModelView):
    # Override the `create_form()` method to customize the fields displayed in the form
    form_columns = ['id','tname','location']
    def create_form(self):
