...
from market_name.forms import RegisterForm, LoginForm1,LoginForm2,Book_Now,SearchForm
...
def book_page(id):
    ...
    if request.method == 'POST':
        ...
        if ns>show.seatno:
            ...
            db.session.commit()
            ticket = Booking(tname=theatre.tname,location=theatre.location,cname=show.sname,srno=show.srno,no_of_seats=n,date=d,
            ...
            db.session.commit()
            return redirect(url_for("complete_page",id=show.id,tc=tc,n=n,t_l=t_l,ist=ist,d=d))
...
def complete_page(id):
    ...
    e=request.args.get('d')
    theatre = Theatre.query.get(show.theatre_id) 
    return render_template('complete.html',theatre=theatre,show=show,a=a,b=b,c=c,d=d,e=e)
...
def search():
    ...
    if  request.method == 'POST':
        ...
        return redirect(url_for("show_result",searched=searched))
...
def show_result():
    ...
    TH = Theatre.query
    T=TH.filter(Theatre.tname.like('%'+ str(searched) + '%')).all()
    L=TH.filter(Theatre.location.like('%'+ str(searched) + '%')).all()
    show = Show.query.filter(Show.sname.like('%' + str(searched) + '%')).all() 
    c1=len(T)
    c2=len(show)
    c3=len(L)

    if c1 ==0 and c2==0 and c3==0:
        return render_template('no_records.html',searched=searched)
    elif c1!=0 and c2==0 and c3==0:
        return render_template("show_theatre.html",T=T, searched=searched)
    elif c3!=0 and c2==0 and c1==0:
        return render_template("show_result.html",L=L, searched=searched)
    else:
        return render_template("show_movie.html",show=show, searched=searched)
